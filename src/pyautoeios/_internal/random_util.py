...
def random_seeds(mod: int = 0, start: int = 8, stop: int = 12):
    ...
    return [
        [random.uniform(0.000, 1.000), random.uniform(0.000, 1.000)]
        for _ in range(sg.randrange(start, stop))
    ]


def random_point_in(x_min, y_min, width, height, seeds: List[List[int]]) -> List[int]:
    """
    Returns a random pixel within some bounding Rectangle based on a list of seeds.
    Args:
        x_min: The left-most coordinate of the bounding Rectangle.
        y_min: The top-most coordinate of the bounding Rectangle.
        width: The width of the bounding Rectangle.
        height: The height of the bounding Rectangle.
        seeds: A list of seeds to use for the randomization.
    Returns:
        A random [x, y] coordinate within the bounding Rectangle.
    """
    sg = secrets.SystemRandom()

    if sg.randrange(0, 101) > 75:
        # Generate a random pixel within the full bounding Rectangle.
        return __random_from(x_min, y_min, width, height)

    # Calculate the dimensions and position of an inner bounding Rectangle within the full bounding Rectangle.
    ...
    # Calculate the dimensions and position of a bounding Rectangle within the inner bounding Rectangle.
    ...
    # Determine the dimensions of the bounding Rectangle within the inner bounding Rectangle.
    inner_inner_width = (
        start_fix_width if start_fix_width <= end_fix_width else end_fix_width
    )
    inner_inner_height = (
        start_fix_height if start_fix_height <= end_fix_height else end_fix_height
    )

    # Generate a random pixel within the bounding Rectangle within the inner bounding Rectangle.
    return __random_from(
        start_x, start_y, inner_inner_width, inner_inner_height, centered=False
    )


def __random_from(x_min, y_min, width, height, centered: bool = True) -> List[int]:
    """
    Helper function to generate a random pixel within some bounding Rectangle. The bounding Rectangle can be
    centered on the x_min and y_min coordinates, or the bounding Rectangle can be offset from the x_min
    ...
    Args:
        ...
        width: The width of the bounding Rectangle.
        height: The height of the bounding Rectangle.
        centered: Whether or not the bounding Rectangle is centered on the x_min and y_min coordinates.
    """
    if centered:
        # The bounding Rectangle to search is to be centered on the x_min and y_min coordinates
...
def truncated_normal_sample(lower_bound, upper_bound, mean=None, std=None) -> float:
    """
    Generate a random sample from a normal distribution using the Rectangle-Muller method.
    Args:
    ...
    while True:
        ...
        z = x1**2 + x2**2
        if z >= 0 and z <= 1:
            # Use the Rectangle-Muller transform to generate a sample from the normal distribution
...
def fancy_normal_sample(lower_bound, upper_bound) -> float:
    ...
    p = [
        (i + 1) ** 2 / sum((i + 1) ** 2 for i in range(len(means)))
